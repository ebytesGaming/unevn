    ...
    <main className="min-h-screen bg-background px-6 py-8 text-foreground">
      ...
      <div className="mx-auto grid min-h-[calc(100vh-7rem)] max-w-5xl items-center gap-16 py-16 lg:grid-cols-[1fr_380px]">
        ...
        <section className="rounded-2xl border border-border bg-surface/75 p-7 shadow-2xl backdrop-blur-xl sm:p-9">
          ...
          <div className="space-y-3"><Button asChild className="h-11 w-full rounded-xl"><a href={`/sign-in?plan=${encodeURIComponent(plan)}`}><Globe2 className="mr-2 size-4" />Continue with Google</a></Button><div className="relative py-3 text-center text-xs uppercase tracking-[0.16em] text-muted-foreground"><span className="relative z-10 bg-surface px-3">or</span><span className="absolute inset-x-0 top-1/2 border-t border-border" /></div><Button type="button" variant="outline" className="h-11 w-full rounded-xl" onClick={() => setMessage("Email sign-in is not connected yet. It will be available here soon.")}><Mail className="mr-2 size-4" />Continue with email</Button></div>
